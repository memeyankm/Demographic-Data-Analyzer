...
def calculate_demographic_data(print_data=True):
    ...
    highest_earning_country = df['native-country'].where(df.salary=='>50k').value_counts().max()
    highest_earning_country_percentage = 0

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df.occupation.where(df['native-country']=='India').where(df.salary=='>50k').value_counts().max()
